...
- **M1b (completed 2026-08-25)**: TeamTemplates execute as fresh deterministic TeamRuns behind the CoordinationSubstrate seam; the local provider and optional `parity-green` ClawTeam provider have cross-platform lifecycle evidence, and the exit-criterion measurement is recorded. Hidden ephemeral members remain M1c work.
- **M1c–M2 (separately planned/unapproved)**: add policy-gated hidden ephemeral members, then nested runs and an `atm` MCP server over the same versioned contracts; declared versus achieved enforcement remains explicit.
