...
- [ ] Prepare and approve a separate plan for TeamTemplate/TeamRun foundations, task/message/archive contracts, and the CoordinationSubstrate protocol
  ...
  - [x] Draft r6 resolves all fifth-review findings (plan §21 r5→r6 table; ADR 0043 — explicit team-member rendering, collision-safe project-local Grok profiles, launch-time target baselines, shared substrate typing, truthful failed/cancelled/abandoned terminal pairings, and Windows refusal). NOT approved — next: owner G0 approval naming the file + r6 commit SHA, or another frozen-SHA confirmation pass
