...
- [ ] Prepare and approve a separate plan for TeamTemplate/TeamRun foundations, task/message/archive contracts, and the CoordinationSubstrate protocol
  ...
  - [x] r4 resolves all seven findings (this commit; plan §21 r3→r4 table; ADR 0041 — member-result pipeline pinned with `parse()` untouched; team-mode target semantics; nullable-until-launch execution bindings; `CLAWTEAM_DISPOSITION`-gated failed branch; snapshot-retention policy; frozen containment allowlist). NOT approved — next: owner G0 approval naming the file + r4's SHA, or another confirmation pass at r4's frozen SHA
