...
- [ ] G5.R — resolve the 2026-08-24 independent-review findings before G6 (`docs/reviews/2026-08-24-g5-review-at-317bb52.md`; ADR 0032); R3–R6/H9 are implemented, but hosted closure awaits the Windows test-only follow-up below
  ...
  - [ ] Hosted CI follow-up — run 32734735405 at `30c17b5` was 7/9 green; both Windows scaffold jobs failed only because the stuck-pipe regression forced a POSIX `os.killpg` monkeypatch. The local test now exercises `_drain_terminated_probe_process` directly and is green with the full core suite; commit, push on fresh approval, and require a 9/9 hosted rerun before re-closing G5.R
