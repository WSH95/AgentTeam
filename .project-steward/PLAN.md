...
- [ ] G5 — deterministic acceptance/fault matrix plus full V1 and cross-platform regression (**local audit green; hosted attempts 32905220326 at `43eaea9` and 32906060190 at `55b04fd` were each 10/12 green. The second run proves the Win32 process-query fix and exposed a separate exclusive-lease read at manifest finalization; excluding ephemeral `controller.lock` is locally green at 35 focused and 737 full-suite tests + 4 expected skips; third hosted run pending**)
- [ ] G6 — current-version no-call capability probes and honest support disposition (**qualification machinery is implemented and fail-closed; the owner approved installing only the exact pinned runtime and running zero-call qualification after G5, but the runtime remains absent until that step**)
