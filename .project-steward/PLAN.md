...
- [ ] G7 — **in progress under ADR 0049**: build and host-validate the internal attended driver plus sanitized-export hardening first, then run Claude → Codex → Grok lifecycle qualification and the four-call workflow; successful ledger must be exactly 19/23 with diagnostics 0, and the first mechanical failure stops the matrix
