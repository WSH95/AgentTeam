...
- [ ] Prepare and approve a separate plan for TeamTemplate/TeamRun foundations, task/message/archive contracts, and the CoordinationSubstrate protocol
  ...
  - [x] r5 resolves all fourth-review findings (this commit; plan §21 r4→r5 table; ADR 0042 — explicit least-privilege workspace access; durable-allocation execution bindings + archive verifier; verified-copy-out cleanup handshake; completion publication barrier; canonical deliverables; occurrence-level containment). NOT approved — next: owner G0 approval naming the file + r5's SHA, or another confirmation pass at r5's frozen SHA
