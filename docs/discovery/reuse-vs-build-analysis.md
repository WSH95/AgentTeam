...
**Cost** (borrow/adapt step only): `h` hours · `d` ≤5 working days · `w` 1–4 weeks · `—` nothing to borrow or build (no cost). **Risk** `low|med|high (factor)` — *merge*: rung 3 depends on HKUDS/ClawTeam (no commit since 2026-05-09, 22 open PRs [ev:clawteam-spawn-platform#F24]); rung 2b on seams without a stability promise ("Alpha", no `__all__` [ev:clawteam-spawn-platform#F15]); rung 2a only on CLI flag drift; fork backends bypass `adapter.prepare_command` (5 `xfail`) [ev:clawteam-openclaw-fork-delta#F27]. *platform*: tmux, Gateway daemon, POSIX-only shell, Windows untested (no Windows CI in CT/CTF/DT/DG [ev:clawteam-spawn-platform#F20][ev:dsh-agent-teams-and-gui §4]). *verif.*: enabling cell is `~`/`?`. **Platform**: needs on Ubuntu (this host, no tmux), macOS, Windows; "any" = pure data. Licences: MIT · Apache · prop. (Claude Code, invocation only) · none (ATM; ideas citable, text not copyable until licensed). Versions evaluated are those of fit-gap §"System columns" (CT 0.3.0@0119833, CTF 0.3.0+openclaw2, OC 2026.7.1-2, CC 2.1.239, CX 0.148.0, Grok 1.0.5, HM 0.20.4); they are repeated in §7.
...
| G-AD-05 (M) | DG S! | **6** strict schema + exclusion validator on the new definition object (the enforcement the wording asks for); 1 run-side isolation knobs keep run facts out of the definition | CC `--bare`; CX `--ephemeral`/`CODEX_HOME`; Grok `GROK_HOME`; HM fresh profile/`--ignore-rules`; OC `--profile`/session key; DG `.strict()` (pattern) | h, low, any | [ev:harness-cli-capabilities-a#F19][ev:harness-cli-capabilities-b#F24][ev:dsh-agent-teams-and-gui#F20] |
...
| G-AD-07 (M) | CT S! (tie: CTF, OC, CC, HM, DT; W3 re-score) | **1** spawn mechanism; hidden = G-TC-04 (6 field + 2 filtered roster; CT flag at 3 optional); object = G-AD-01 | HM `delegate_task`/`--source tool`; CC `--agents` + background subagents; OC `sessions_spawn`; CT `spawn` (MIT/prop.) | d, low (rung 2 shadow; merge risk only if the CT PR is pursued), OC needs Gateway | [ev:claude-agent-teams-hermes-openbot#F22][ev:harness-cli-capabilities-b#F12] |
...
| G-TC-03 (S) | DG C!; OC C! | **6** enforcement-level field (`advisory` / `mechanical`) on the new template object — the wording asks the template to declare it; 1 by construction (reviewer = own HarnessInvocation, fresh context, no inbox); 3 CT inbox ACL (optional, for the mechanical level) | CT subprocess workers; HM profiles; CC `isolation: worktree`; DG `contextMode=spawn` (MIT); ATM ADR 0026 (none) | d, low (rung 1 mechanism); med (merge) only for the ACL PR, any | [ev:clawteam-model#F4][ev:dsh-agent-teams-and-gui#F25] |
...
| G-AR-05 (S) | OC S! | **6** per-capability report (small); 1 inputs | OC `skills list --json` eligibility; HM `env_requires`; CT `profile test` (MIT); ATM outcome vocabulary `native` / `artifacts[]` / `emulated` / `unsupported` (none) | d, low, any | [ev:harness-cli-capabilities-b#F11][ev:atm-salvage#F10][ev:atm-salvage#F7] |
...
| DG `src/tools/{recipes.ts 122, storage-transaction.ts 42, run-history.ts 275, orchestration.ts 173}`, `types.ts`/`spec.ts` zod records; DT `src/state.ts` (882), `types.ts` (104) (line counts = `wc -l` on `dsh-agent-team-gui/src/tools/*.ts`, `dsh-agent-teams/src/{state,types}.ts`) | pure (DG: only a `KvTable` type; DT: node only) [ev:dsh-agent-teams-and-gui#F31] | MIT | d (TypeScript; language TBD) | AR-03 export/import preview+rollback, TC-02 bounded handoffs, TE-06 plan validation, TE-07/HB-07 run records; TE-06/TE-07 JSONL mailbox, atomic write with Windows retry, archive-on-delete |
...
| Claude Code (CC) | 2.1.239 | proprietary — Commercial/Consumer terms (web, accessed 2026-08-21) | invocation/config only; API key for automation | [ev:harness-cli-capabilities-a#F15] |
| Codex CLI (CX) | 0.148.0 | Apache-2.0 — github.com/openai/codex README (web, accessed 2026-08-21); LICENSE file unfetched | yes (pending file) | [ev:harness-cli-capabilities-a#F14] |
...
| ATM | `agent-team-manager-dev@12a727e` (0.0.0, private) | **none** — no `LICENSE`, no `license` field | text blocked until the owner states one; ideas citable | [ev:atm-salvage §3] |
