...
status: draft v2.1 — current harness versions/isolation and AD-07 correction applied
date: 2026-08-22
...
This document models the **Assistant** — the product's primary reusable object (product-intent §1.2) — precisely enough that later documents can map it onto substrates. It owns the content model/exclusion list, Assistant/AssistantInstance boundary, capability/artifact requirements, harness preferences as data, the Ephemeral Assistant definition, and evolution overlays. Current local versions are Claude Code 2.1.241, Codex 0.149.0, Grok Build 1.0.5, OpenClaw 2026.7.1-2, Hermes 0.20.4, and ClawTeam 0.3.0@0119833 [ev:m0-product-architecture-review-2026-08-22#F1].
...
The instantiation boundary is the moment an effective definition is rendered for one harness. ClawTeam generates a session id and resumes only explicitly; Claude native subscription runs use `--safe-mode --no-session-persistence` (`--bare` disables OAuth); Codex uses `--ephemeral`; Grok needs isolated home/memory controls; OpenClaw/Hermes freshness is materialized by a new session key/profile [ev:m0-product-architecture-review-2026-08-22#F3][ev:harness-cli-capabilities-b#F19].
...
Today's substrates create ephemeral workers, but as **task text without a portable definition object**. The targeted AD-07 review also restores the full hidden-if-desired clause: OpenClaw's non-user-roster child path is the best complete native behavior; ClawTeam/its fork/dsh-teams need a visibility extension, Claude composes background subagents, and Hermes/Codex visibility semantics remain unverified. The run layer must therefore own both the definition object and the user-visible/archive projections [fitgap:AD-07][ev:m0-product-architecture-review-2026-08-22#F9].
...
| AssistantInstance (fresh) | spawn with new `--session-id`; `--resume` opt-in [ev:clawteam-spawn-platform#F19] | `-p --safe-mode --no-session-persistence` | `exec --ephemeral` | new `--session-key` on a persistent OpenClaw agent, or disposable `--profile` [ev:atm-salvage#F4][ev:harness-cli-capabilities-b#F10] | `profile create --clone` or `HERMES_HOME=<tmp>` [ev:harness-cli-capabilities-b#F19] | teams: continuable child [ev:dsh-agent-teams-and-gui#F4]; gui: fresh child per dispatch [ev:dsh-agent-teams-and-gui#F21] |
...
| Fresh-by-default instance | ClawTeam `--session-id`/`--resume`; Claude `--safe-mode --no-session-persistence`; Codex `--ephemeral`; isolated Grok state [ev:m0-product-architecture-review-2026-08-22#F3] | borrowed (adapter-materialized on OpenClaw/Hermes) |
...
7. Which owner-authorized ATM code or text, if any, is technically worth adapting? Preserve provenance and third-party notices; the owner's permission does not choose a public ATS licence.
