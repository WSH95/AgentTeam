...
| **CoordinationSubstrate** *(added 2026-08-23; amended 2026-08-24 at M1b G0)* | The second declared seam: the protocol through which a TeamRun obtains team/member/task/wait/message/snapshot/cleanup operations from an optional coordination provider (a local deterministic provider first; ClawTeam as the first external provider). Process stopping is a run-layer duty and always precedes provider cleanup; the provider never launches, supervises, or stops harnesses. Documented in M1a, implemented from M1b. | "substrate" alone (Substrate is the wider noun), "backend" |
