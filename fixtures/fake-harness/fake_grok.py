...
semantic-miss); $FAKE_MODE_GROK overrides it for this vendor only. A
synthesis-shaped invocation (the requested schema names `synthesis-report`)
makes the fake parse the labelled-reports document out of its prompt file and
  ...
  "findings": [
    {
      ...
      "file": "src/publish.ts",
      "line": 5,
      "title": "Unescaped input reaches the shell",
      "rationale": "publishTag builds a git command from raw tag and remote."
    },
    {
      "id": "f2",
      "severity": "low",
      ...
      "line": 4,
      "title": "Boundary reads past the requested count",
      "rationale": "The loop condition uses <= so one extra entry is read."
    }
  ],
